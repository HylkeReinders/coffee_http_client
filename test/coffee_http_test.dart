...
void main() {
  group('CoffeeUri', () {
    test('build() normalizes empty prefix and empty path to "/"', () {
      final uri = CoffeeUri(host: 'example.com', scheme: CoffeeHttpScheme.https, prefix: '');
    ...
    test('build() normalizes prefix without leading slash', () {
      final uri = CoffeeUri(host: 'example.com', scheme: CoffeeHttpScheme.https, prefix: 'api/v1');
    ...
    test('build() normalizes path without leading slash', () {
      final uri = CoffeeUri(host: 'example.com', scheme: CoffeeHttpScheme.https, prefix: '/api');
    ...
    test('build() attaches query parameters', () {
      ...
      expect(built.toString(), 'https://example.com/api/products?page=2&limit=20');
    });
  });

  group('CoffeeRequest', () {
    test('defaults are empty collections and null jsonBody', () {
      final req = CoffeeRequest(method: CoffeeHttpMethod.get, path: '/users/me');
  ...
  group('CoffeeRawResponse', () {
    ...
    test('copyWith can update duration', () {
      final original = CoffeeRawResponse(statusCode: 200, headers: const {}, body: '', duration: Duration.zero);
  ...
  group('CoffeeHooks', () {
    test('handleResponse returns value that can be cast by the caller', () {
      ...
      final ctx = CoffeeHandleResponseContext(
        ...
        response: CoffeeRawResponse(statusCode: 200, headers: const {}, body: '{"ok":true}', duration: const Duration(milliseconds: 5)),
      );
    ...
    test('onResponse and onError are optional and callable', () {
      ...
      hooks.onResponse!(
        CoffeeResponseContext(
          ...
          response: CoffeeRawResponse(statusCode: 200, headers: const {}, body: '', duration: Duration.zero),
        ),
      );
      ...
      hooks.onError!(
        CoffeeErrorContext(
          ...
          error: CoffeeHttpError(kind: CoffeeHttpErrorKind.network, underlying: Exception('boom')),
        ),
      );
  ...
  group('CoffeeHttpError', () {
    test('stores kind and underlying exception', () {
      ...
      final err = CoffeeHttpError(kind: CoffeeHttpErrorKind.network, underlying: ex);
    ...
    test('supports timeout kind', () {
      final err = CoffeeHttpError(kind: CoffeeHttpErrorKind.timeout, underlying: TimeoutException('timed out'));
